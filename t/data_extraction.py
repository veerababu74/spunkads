...
class ManyChat_CSV_Processor:
    ...
    def fetch_revenue_for_extracted_pages(self, extracted_page_ids: list) -> dict:
        """Fetch revenue data from SpunkStats API for pages that were actually extracted"""
        print("🔄 Fetching revenue data from SpunkStats API for extracted pages...")
        revenue_data = {}
        ...
        for page in self.pages_full_data:
            ...
            if page_id in extracted_page_ids:
                ...
                if self.config.get("verbose", True):
                    print(f"   📋 Found match: {page.get('name')} (ID: {page_id})")
        ...
        if not user_id or not api_key:
            ...
            for page in pages_to_process:
                ...
                revenue_data[page_name] = {"revenue": "0.00", "timestamp": "N/A"}
            return revenue_data
        ...
        try:
            ...
            if api_revenue_data:
                if self.config.get("verbose", True):
                    print(
                        f"   📊 Processing API response for {len(pages_to_process)} pages..."
                    )
                ...
                for page in pages_to_process:
                    ...
                    if revenue_info:
                        ...
                        if self.config.get("verbose", True):
                            print(
                                f"   💰 {page_name}: ${revenue_info['revenue']} (Date: {revenue_info['timestamp']})"
                            )
                    else:
                ...
                # API call failed, set all pages to 0.00
                for page in pages_to_process:
                    page_name = page.get("name", "")
                    revenue_data[page_name] = {"revenue": "0.00", "timestamp": "N/A"}
                if self.config.get("verbose", True):
                    print(f"   💰 API call failed - all pages set to $0.00")

        except Exception as e:
            print(f"❌ Error fetching revenue data: {e}")
            # Set all pages to 0.00 on error
            for page in pages_to_process:
                page_name = page.get("name", "")
                revenue_data[page_name] = {"revenue": "0.00", "timestamp": "N/A"}

        print(f"✅ Revenue data fetched for {len(revenue_data)} pages")
        return revenue_data
    ...
    def extract_revenue_and_timestamp_for_page(
        ...
        """Extract revenue and timestamp for specific page from SpunkStats API response"""
        try:
            ...
            # Collect all revenue values for this page name
            total_revenue = 0.00
            timestamp = "N/A"
            matching_rows = []
            ...
            for i, row in enumerate(data_array):
                if isinstance(row, dict):
                    # New API format: row is a dictionary with keys like 'dt', 'utm_s', 'a'
                    utm_s = str(row.get("utm_s", "")).strip()

                    if self.config.get("verbose", True) and i < 10:
                        print(
                            f"     Row {i}: utm_s='{utm_s}' | Date='{row.get('dt')}' | Revenue='{row.get('a')}'"
                        )
                    ...
                    if utm_s == page_name:
                        ...
                        matching_rows.append(
                            {"row": i, "revenue": revenue, "data": row}
                        )

                        if self.config.get("verbose", True):
                            print(
                                f"   ✅ MATCH FOUND! '{page_name}' matches utm_s '{utm_s}' at row {i}: revenue = ${revenue}"
                            )

            if matching_rows:
                if self.config.get("verbose", True):
                    print(
                        f"   📊 Total revenue for {page_name}: ${total_revenue:.2f} from {len(matching_rows)} rows"
                    )
                    print(f"   📅 Using timestamp: {timestamp}")

                return {"revenue": f"{total_revenue:.2f}", "timestamp": timestamp}
    ...
    def get_page_details(self, page_id: str) -> dict:
        """Get page details (account_name, user, tl, revenue, revenue_timestamp) for a given page_id"""
        ...
        # Add revenue data and timestamp from SpunkStats API
    ...
    def process_json_to_csv(self, json_files: List[str]) -> Dict[str, Any]:
        ...
        if all_summary_data and self.output_settings["include_summary_csv"]:
            summary_file = self.create_summary_csv(all_summary_data)
    ...
    def create_summary_row(self, page_info: Dict, posts: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            user = page_details["user"]
            tl = page_details["tl"]
            revenue = page_details["revenue"]
            revenue_timestamp = page_details["revenue_timestamp"]
